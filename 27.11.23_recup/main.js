...
async function fetchData(){
    return new Promise((resolve, reject) => {
        randomNumber === 1 ? resolve("Data Retrieved") : reject("Error in Data Retrieval ex2");
...
const callForFunction = async () => {
    console.log(111)
    try{
        await fetchData().then(()=>{console.log("I recive data and show this message")})
    //    console.log(()=>{fetchData()}) 
    } catch (error) {
        console.log("call #2 of function:", error)
    }
}
callForFunction()
/*
...
const promise3 = new Promise((resolve, reject) => {
    ...
    Math.floor(Math.random()*2) >= 1 ? setTimeout(resolve, 3000, 'promise3') : setTimeout(reject, 3000, 'promise3 was rejected');
});
